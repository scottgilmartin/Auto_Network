...
def detect_chars():
    ...
    for i in range(len(w)):
        if w[i].isupper() and len(w[i]) > 1 and all(['KHAL' not in w[i],'THE' not in w[i]]): #extract the list of all uppercase words from script
            caps.append(w[i])
            
    chars=list(set([cap for cap in caps if caps.count(cap)>threshold and len(cap)>1])) #generate the character list
    return chars
            ...
            if chars[n].lower().capitalize()==w[i] or chars[n]==w[i]:  #if the current character is found in the string list as the ith string...
                ...
                for m in range(len(chars)): #for each character in the char list...
                     ...
                     for j in range(10, -16, -1): #in the specified range...
                         ...
                         if chars[m].lower().capitalize() == w[i-j] or chars[m]==w[i-j] and m!=n: #if character m is found j words away from  character n = w[i]...
...
ran_hex_lst=['#%02X%02X%02X' % (r(),r(),r()) for _ in range(len(chars))] #randomly generated node colors, use for large char lists 
...
#[nx.shortest_path_length(G, i, j) for i,j in range(len(chars))] #use this when you know whole graph is connected e.g. for a play or sitcom
